...
public class FuncionPublisher_APIMQTT implements MqttCallback {
	...
	protected FuncionPublisher_APIMQTT(IDispositivo dev) {
	...
	public void messageArrived(String topic, MqttMessage message) throws Exception {
		...
		String dispositivoId = topicNiveles[topicNiveles.length-2];
		
		IDispositivo d = this.dispositivo;
		if ( d == null ) {
			MySimpleLogger.warn(this.loggerId, "No encontrada funcion " + dispositivoId);
			return;
		}
		...
		if ( action.equalsIgnoreCase("habilitar") ){
			MySimpleLogger.warn(this.loggerId, "habilitar " + dispositivoId);
			d.setActivePlus("Habilitado");
		}
		else if ( action.equalsIgnoreCase("deshabilitar") ){
			MySimpleLogger.warn(this.loggerId, "deshabilitar " + dispositivoId);
			d.setActivePlus("DesHabilitado");
		}
		else
...
//			connOpt.setPassword(M2MIO_PASSWORD_MD5.toCharArray());
	...
	protected String calculateInfoTopic(IFuncion f) {
		return Configuracion.TOPIC_BASE + "dispositivo/" + dispositivo.getId() + "/info";
	}
