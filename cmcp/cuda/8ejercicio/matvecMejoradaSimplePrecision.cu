...
#include <sys/time.h>
#define BS 256
/* Simple precision no es mas que cambiar fouble por float y cambiar 
 la precision del error.*/
typedef struct {
...
void verify(int n,float *A, float *x, float *y)
{
  ...
  if (err/nrm>1e-5) printf(" relative error = %g\n",err/nrm);
  else printf(" PASS\n");
  free(z);
}
...
float randvalue()
{
  return (float)((rand()/(float)((unsigned int)RAND_MAX+1)))-0.5;
}
