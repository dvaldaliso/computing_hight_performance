...
int cholesky( int n, int b, double *C ) {
  ...
  //Este bucle no es paralelizable
  for ( k = 0; k < n ; k+=b ) {
    m = min( n-k, b );
    ...
    for ( ii = 0; ii < (n-k-1)/b; ii++ ) {
      //1 opcion Crear clase funtor para parelizar
      //Este ciclo recorre la base triangular inferior de la matriz
      //pag 28/75 conferencia
      //i>j se hace la geam  
      //i==j se hace la syrk
      //i<j nada  
      i = k+b+ii*b;
