...
int cholesky( int n, int b, double *C ) {
  ...
  for ( k = 0; k < n ; k+=b ) {
    ...
    //Capturo variables por referencia
    parallel_for( blocked_range<int>(0,(n-k-1)/b), [&]( const blocked_range<int>&r){
      for( size_t ii=r.begin(); ii!=r.end(); ii+=1 ){
      //for ( ii = 0; ii < (n-k-1)/b; ii++ ) {
        //i,m son variables locales
        int i = k+b+ii*b;
        int m = min( n-i, b );
    ...
    parallel_for( blocked_range<int>(0,(n-k-1)/b), [&]( const blocked_range<int>&r){
      ...
      for( size_t ii=r.begin(); ii!=r.end(); ii+=1 ){
        i = k+b+ii*b;
        m = min( n-i, b );
        parallel_for( blocked_range<int>(0,(n-k-1)/b), [&]( const blocked_range<int>&s){
        //for ( jj = 0; jj < (i-k-1)/b; jj++ ) {
          for( size_t jj=s.begin(); jj!=s.end(); jj+=1 ){
            j = k+b+jj*b;
