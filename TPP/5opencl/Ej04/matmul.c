...
int main(int argc, char *argv[])
{
    ...
    if (argc < 3) {
      printf("Usage: ./mult <matrix_size> <work_group_size> [--list] [--device <device_id>]\n");
      exit(0);
    }
    N = atoi(argv[1]);
...
//--------------------------------------------------------------------------------
    ...
    printf("\nUsing OpenCL device index: %d name: %s\n", deviceIndex, name);
...
//--------------------------------------------------------------------------------
    ...
    kernelsource = getKernelSource("elem.cl");
    // Create the comput program from the source buffer
    program = clCreateProgramWithSource(context, 1, (const char **) & kernelsource, NULL, &err);
    checkError(err, "Creating program with elem.cl");
    free(kernelsource);
    // Build the program
    ...
    checkError(err, "Creating kernel from elem.cl");
    ...
    for (int i = 0; i < COUNT; i++)
    {
        ...
        const size_t local[2] = { (size_t) atoi(argv[2]), (size_t) atoi(argv[2]) };
