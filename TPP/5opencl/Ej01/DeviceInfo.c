...
int main(void)
{
    ...
    for (int i = 0; i < num_platforms; i++)
    {
        ...
        for (int j = 0; j < num_devices; j++)
        {
            ...
            // Get Max clock frequency EJERCICIO 1)D
            size_t freq;
            err = clGetDeviceInfo(device[j], CL_DEVICE_MAX_CLOCK_FREQUENCY, sizeof(cl_ulong), &freq, NULL);
            checkError(err, "Getting MAx clock frequency");
            printf("\t\tMax clock: %lu MB\n", freq);
